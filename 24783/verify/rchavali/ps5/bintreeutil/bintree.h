...
public:
	...
	bool RotateLeftHelper(Node* currNode)
	{
		auto nodePtr=currNode;
		if(nullptr!=nodePtr && nullptr!=nodePtr->right)
		{
			auto rightPtr=nodePtr->right;
			auto leftOfRight=nodePtr->right->left;
	...
	bool RotateRightHelper(Node* currNode)
	{
		auto nodePtr=currNode;
		if(nullptr!=nodePtr && nullptr!=nodePtr->left)
		{
...
public:
	...
	{
		auto tail_of_vine = root;
		while (tail_of_vine != nullptr)
		{
			if (tail_of_vine -> left != nullptr)
			{
				RotateRightHelper(tail_of_vine);
				tail_of_vine = tail_of_vine -> up;
			}
			else
			{
				tail_of_vine = tail_of_vine -> right;
			}
		}
	}
	...
	void Compress(int n)
	{
		...
		while (shift < n)
		{
			...
			// 	RotateLeftHelper(vine_index);					
			// 	vine_index = vine_index->right;
			// 	vine_index = vine_index->up;
			// 	vine_index = vine_index->right;
			// }
...
protected:
	void Rebalance (Node * currNode)
	{
		...
		{
			left_height = currNode->left->height;
			// printf("%d/n",left_height);
		}
		if (currNode -> right != nullptr)
		{
			right_height = currNode->right->height;
			// printf("%d/n",right_height);
		}
		...
		else
		{
			if(right_height > left_height)
			{
				...
				{
					right_right_height = currNode->right->right->height;
					// printf("%d/n",right_right_height);

				}
				if (currNode->right->left != nullptr)
				{
					right_left_height = currNode->right->left->height;
					// printf("%d/n",right_left_height);

				}

				if (right_right_height > right_left_height)
				{
					RotateLeftHelper(currNode);
					return;
				}
				else
				{
					RotateRightHelper(currNode->right);
					RotateLeftHelper(currNode);
					return;
				}
			}
			else
			{
				...
				{
					left_right_height = currNode->left->right->height;
				}
				if (currNode->left->left !=nullptr)
				{
					left_left_height = currNode->left->left->height;
				}

				if (left_left_height > left_right_height)
				{
					RotateRightHelper(currNode);
					return;
				}
				else
				{
					RotateLeftHelper(currNode->left);
					RotateRightHelper(currNode);
					return;
				}
			}
