...
protected:
	...
	std::vector<YsVec3> myVtxVector;
	std::vector<YsVec3> myVtxVector_2;
...
/* virtual */ void FsLazyWindowApplication::Initialize(int argc,char *argv[])
{
	...
	if(2<=argc)
	{
		std::vector <YsVec3> vtx_vector;
		vtx_vector = MakeNACA4DigitAirfoil(std::stoi(argv[1]), std::stod(argv[2]));
		myVtxVector = vtx_vector;
	}
}
...
/* virtual */ void FsLazyWindowApplication::Draw(void)
{
	...
	for (int i = 0; i < myVtxVector.size(); i++)
	{
		glVertex3d(myVtxVector[i][0], myVtxVector[i][1],myVtxVector[i][2]);
	}

	for (int i = 0; i < myVtxVector_2.size(); i++)
	{
		glVertex3d(myVtxVector_2[i][0], myVtxVector_2[i][1],myVtxVector_2[i][2]);
	}
