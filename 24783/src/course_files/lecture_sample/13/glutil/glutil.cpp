...
{
	double wx=((vp.x()+1.0)/2.0)*(double)winWid;
	double wy=((1.0-vp.y())/2.0)*(double)winHei;
	return YsVec2i((int)wx,(int)wy);
}

YsVec3 WindowToViewPort(int winWid,int winHei,int x,int y)
{
	double vx=((double)x/(double)winWid)*2.0-1.0;
	double vy=1.0-((double)y/(double)winHei)*2.0;
