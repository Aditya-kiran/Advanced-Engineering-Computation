...
protected:
	...
	std::vector<YsShellExt> robots_meshes;
...
void FsLazyWindowApplication::RemakeVertexArray(void)
{
	...
	for (int i = 0; i < robots_meshes.size(); i++) {

		for(auto plHd=robots_meshes[i].NullPolygon(); true==robots_meshes[i].MoveToNextPolygon(plHd); )
		{
			auto plVtHd=robots_meshes[i].GetPolygonVertex(plHd);
			auto plCol=robots_meshes[i].GetColor(plHd);
			auto plNom=robots_meshes[i].GetNormal(plHd);
			...
			if(3==plVtHd.size())
			{
				for(int i=0; i<3; ++i)
				{
					auto vtPos=robots_meshes[i].GetVertexPosition(plVtHd[i]);
					vtx.push_back(vtPos.xf() + robot_x[i]);
					vtx.push_back(vtPos.yf() + robot_y[i]);
					vtx.push_back(vtPos.zf());
					nom.push_back(plNom.xf());
					nom.push_back(plNom.yf());
					nom.push_back(plNom.zf());
...
/* virtual */ void FsLazyWindowApplication::Initialize(int argc,char *argv[])
{
	if(2<=argc && true==mesh.LoadStl(argv[1]))
	{
		...
		for (int i = 0; i < 3; i++) {
			...
			YsShellExt temp_mesh;
			temp_mesh.LoadStl(argv[1]);
			robots_meshes.push_back(temp_mesh);
		}
