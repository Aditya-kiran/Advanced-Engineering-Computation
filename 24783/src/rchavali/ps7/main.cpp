...
protected:
	...
	int state = 0; // 0=Paint, 1=slice, 2=laser
...
/* virtual */ void FsLazyWindowApplication::Interval(void)
{
	...
	{
		state = 2;
		rayItscVtx = MakeRayIntersectionVertexArray(mesh);
	}
...
/* virtual */ void FsLazyWindowApplication::Draw(void)
{
	...
	{
		glColor3ub(0,0,0);	
		for (int i=0; i<sliceVtx.size()-6; i+=6)
		{
			glBegin(GL_LINES);
			glVertex3f(sliceVtx[i],sliceVtx[i+1],sliceVtx[i+2]);
			glVertex3f(sliceVtx[i+3],sliceVtx[i+4],sliceVtx[i+5]);
			glEnd();
		}
	...
	{
		glColor3ub(0,0,0);
		for (int i=0; i<rayItscVtx.size()-3; i+=3)
		{
			glPointSize(4);
			glBegin(GL_POINTS);
			glVertex3f(rayItscVtx[i],rayItscVtx[i+1],rayItscVtx[i+2]);
			glEnd();
		}
