...
protected:
	...
	void DrawCircle(int cx,int cy,int rad,int fill)
	{
	    ...
	    glEnableVertexAttribArray(plain2d.attribVertexPos);
	    glVertexAttribPointer(plain2d.attribVertexPos,2,GL_FLOAT,GL_FALSE,0,coord.data());
	    if(0!=fill)
	    {
		    glDrawArrays(GL_TRIANGLE_FAN,0,coord.size()/2);
	    }
	    else
	    {
		    glDrawArrays(GL_LINE_LOOP,0,coord.size()/2);
	    }
	    glDisableVertexAttribArray(plain2d.attribVertexPos);
	}
	...
	void DrawRect(int x1,int y1,int x2,int y2,int fill)
	{
	    ...
	    glEnableVertexAttribArray(plain2d.attribVertexPos);
	    glVertexAttribPointer(plain2d.attribVertexPos,2,GL_FLOAT,GL_FALSE,0,coord);
	    if(0!=fill)
	    {
	...
	void DrawArtillery(double x,double y,double angle)
	{
		...
		glVertexAttrib4f(plain2d.attribColorPos,0,0,1,1);
	...
	void DrawCannonBall(double x,double y)
	{
		...
		glVertexAttrib4f(plain2d.attribColorPos,1,0,0,1);
	...
	{
	    int i;
		glVertexAttrib4f(plain2d.attribColorPos,0,1,0,1);
	    for(i=0; i<nObstacle; i++)
	    {
	...
	void DrawTrajectory(int nPnt,const double pnt[])
	{
		if(2<=nPnt)
		{
			glVertexAttrib4f(plain2d.attribColorPos,0,0,0,1);
			std::vector <float> vtx;
			for(int i=0; i<nPnt; i++)
			{
				int sx,sy;
				PhysicalCoordToScreenCoord(sx,sy,pnt[i*2],pnt[i*2+1]);
				vtx.push_back((float)sx);
				vtx.push_back((float)sy);
			}
		    glEnableVertexAttribArray(plain2d.attribVertexPos);
		    glVertexAttribPointer(plain2d.attribVertexPos,2,GL_FLOAT,GL_FALSE,0,vtx.data());
...
/* virtual */ void FsLazyWindowApplication::Initialize(int argc,char *argv[])
{
	...
	plain2d.CompileFile("plain2d_vertexShader.glsl","plain2d_fragmentShader.glsl");
}
...
/* virtual */ void FsLazyWindowApplication::Draw(void)
{
    ...
    glUseProgram(plain2d.programIdent);
