...
void LMap::ReadMap()
{
    ...
    for (int i = 0; i < this->travel_time_list.size(); i++) {
        for (int j = 0; j < this->travel_time_list[i].size(); j++) {
            if (this->travel_time_list[i][j] > 2) {
                this->travel_time_list[i][j] = 10;
            }
        }
    }
...
void LMap::StoreStraddlePath(int straddle_id, route curr_route) {
    ...
    redisReply *reply = (redisReply *)redisCommand(ctx, "HSET straddle_path %d %s", straddle_id, vps_xy_str.c_str());
    freeReplyObject(reply);
    redisReply *reply_vps = (redisReply *)redisCommand(ctx, "HSET straddle_vpoint %d %s", straddle_id, vps_str.c_str());
    freeReplyObject(reply_vps);
    redisReply *reply_wts = (redisReply *)redisCommand(ctx, "HSET straddle_waittime %d %s", straddle_id, wts_str.c_str());
    freeReplyObject(reply_wts);
    redisReply *reply_tts = (redisReply *)redisCommand(ctx, "HSET straddle_traveltime %d %s", straddle_id, tts_str.c_str());
    freeReplyObject(reply_tts);
    redisReply *reply_arr_and_depar_time = (redisReply *)redisCommand(ctx, "HSET straddle_arr_and_depar_time %d %s", straddle_id, arr_and_depar_time_str.c_str());
    freeReplyObject(reply_arr_and_depar_time);
}

route LMap::RetriveStraddlePath(int straddle_id) {
    ...
    redisReply *reply_vps = (redisReply *)redisCommand(ctx, "HGET straddle_vpoint %d", straddle_id);
    vps_str = reply_vps->str;
    freeReplyObject(reply_vps);
    redisReply *reply_wts = (redisReply *)redisCommand(ctx, "HGET straddle_waittime %d", straddle_id);
    wts_str = reply_wts->str;
    freeReplyObject(reply_wts);
    redisReply *reply_tts = (redisReply *)redisCommand(ctx, "HGET straddle_traveltime %d", straddle_id);
    tts_str = reply_tts->str;
    freeReplyObject(reply_tts);
    redisReply *reply_arr_and_depar_time = (redisReply *)redisCommand(ctx, "HGET straddle_arr_and_depar_time %d", straddle_id);
...
std::vector<std::vector<double> > LMap::ReadFastMap() {
    int x, y;
    double val;
    std::ifstream fin("number_storage", std::ios_base::binary);
...
double LMap::getFastMapDistance(int node1, int node2) {
    int layer_number = this->fastmap.size();
    double distance = 0;
    for (int i = 0; i < layer_number; i++)
        distance += fabs(this->fastmap[i][node1] - this->fastmap[i][node2]);
    return distance;
}
...
void LMap::FunctionTest() {
    ...
    if (debug4LockTimeWindow) {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            curr_node->travel_time_from_parent = 1;    std::cout << "let's play, honey!" << std::endl;
            openList.insert(make_pair(90 - i, curr_node));
            closeList.push_back(curr_node);
            cnt += 1;
        }
    ...
    if (debug4MultiPP) {
        ...
        std::vector<int> straddle_numbers;
        //straddle_numbers.push_back(1);
        straddle_numbers.push_back(1);
        straddle_numbers.push_back(2);
        ...
        from_ids.push_back(0);
        from_ids.push_back(418);
        std::vector<int> to_ids;
        to_ids.push_back(155);
        to_ids.push_back(14);
        result = this->multi_pp(straddle_numbers, from_ids, to_ids, start_work_time);
        double end = time(NULL);
        for (int i = 0; i < result.size(); i++) {
            std::cout << "Vihicle ID : " << straddle_numbers[i] << std::endl;
            std::cout << "vps:" << result[i].vps_str << std::endl;
            std::cout << "wts:" << result[i].wts_str << std::endl;
            std::cout << "tts:" << result[i].tts_str << std::endl;
            std::cout << std::endl;
            this->StoreStraddlePath(i + 1, result[i]);
        }
...
void LMap::PrintStraddlePath(int straddle_id, route curr_route) {
        std::cout << "Straddle ID : " << straddle_id << std::endl;
...
void LMap::initLockArea(std::vector<int> needToInit) {
    for (int i = 0; i < needToInit.size(); i++) {
        int straddle_id = needToInit[i];
        route r = this->RetriveStraddlePath(straddle_id);
...
std::vector<route> LMap::serve(std::vector<int> straddle_numbers, std::vector<int> from_ids, std::vector<int> to_ids, std::vector<double> start_working_time) {

    this->init();
    std::vector<route> result;

    std::vector<int> needToInit;
    for (int i = 1; i <= this->straddle_number; i++) {
        bool flag = true;
        for (int j = 0; j < straddle_numbers.size(); j++) {
            if (straddle_numbers[j] == i) {
                flag = false;
                break;
            }
        }
    ...
    this->GetStraddlePositions();
    this->initLockArea(needToInit);
    result = multi_pp(straddle_numbers, from_ids, to_ids, start_working_time);
    if (result.size() == straddle_numbers.size()) {
        for (int i = 0; i < straddle_numbers.size(); i++) {
            //this->StoreStraddlePath(straddle_numbers[i], result[i]);
        }
    }
    return result;
}

std::vector<route> LMap::multi_pp(std::vector<int> straddle_numbers, std::vector<int> from_ids, std::vector<int> to_ids, std::vector<double> start_working_time) {
    std::vector<route> result;
    //this->init();
...
//        temp_legal_time = this->getLegalInterVals4Node(startNode->node_id);
        ...
        if (startNode->feasible_arrival_time.size() == 0) {
            ...
            for (int i = 0; i < straddle_numbers.size(); i++) {
                route onePointRoute;
                int straddle_id = straddle_numbers[i];
                int currPos = this->straddle_positions[straddle_id - 1];
                std::vector<int> vps;
                std::vector<double> wts;
                std::vector<double> tts;
                std::vector<std::pair<double, double>> arr_and_depar_time;
...
void LMap::LockTimeWindow4Area(route input) {
    ...
    curr_arr_time = arr_and_depar_time[0].first;
    wait_time = 0;
    for (int i = 0; i < vps.size() - 1; i++) {
        int link_id = this->link_id_map[vps[i]][vps[i + 1]];
        //std::vector<int> curr_node_set = this->link_lock_node_list[link_id];
        std::vector<int> curr_node_set = this->lock_node_list[vps[i]];
        std::vector<int> next_node_set = this->lock_node_list[vps[i + 1]];

        wait_time = wts[i];
        double begin_time = curr_arr_time + wait_time;
        double end_time = curr_arr_time + wait_time + tts[i] + this->time_stamp;


        if (this->getDistance(vps[i], vps[i + 1]) > 1.01) {
            for (int j = 0; j < curr_node_set.size(); j++) {
                LockTimeWindow4Node(curr_node_set[j], begin_time, end_time + 4);
            }
            for (int j = 0; j < next_node_set.size(); j++) {
                //Todo this is hard-coding, fix it later
                LockTimeWindow4Node(next_node_set[j], begin_time + 3, end_time + 4);
            }
        }
        curr_arr_time += tts[i];
        curr_arr_time += wait_time;
...
void LMap::GetStraddlePositions() {
...
//std::vector<std::pair<double, double> > LMap::getLegalInterVals4FarNodes(int curr_node_id, int prev_node_id) {
//     curr_legal_time = this->getLegalInterVals4Node(curr_node_id);
//     prev_legal_time = this->getLegalInterVals4Node(prev_node_id);
//}
